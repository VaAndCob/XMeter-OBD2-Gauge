...
;Ver 1.06 Date 23/03/2013 {Mode 22 PID 1674 Transmission Fluid Temp} not finish yet.
;==============================================
;Pin Configuration
...
;CALL	WRSTR		;write D char in CGRAM  06h
;MOV	DPTR,#CGM	;Load 3 char
;CALL	WRSTR		;write 3 char in CGRAM  07h
MOV	DPTR,#CG3	;Load 3 char
CALL	WRSTR		;write 3 char in CGRAM  06h
MOV	DPTR,#CGX	;Load X Char
CALL	WRSTR		;write X char in CGRAM 07h
...
CALL	AIRFLOW		;Same Fomula as Air Flow
RET
...
CJNE	A,#34h,PWRDOWN	;If A = "4" then next else jump
MOV	SLEEPCNT,#24h	;Reset sleep mode counter
...
TITLE1:DB  00h,01h,02h," OBDII ",07h,"-METER V1",0Dh
TITLE2:DB  03h,04h,05h," ----------------",0Dh
TITLE3:DB  "By Jerry @ThaiMazda",06h,0Dh,00h

MENU1:DB "Select SETUP Menu[~]",0Dh,00h
...
MENU17: DB "7. Firmware Ver 1.05",0Dh,00h
RELEASE: DB "Jan05,2013|JavaJacob",00Dh,00h

MENU2:DB "Special Function [~]",0Dh,00h
VEHINFO:DB "1. Veh Info Numbers:",0Dh,00h
DTC:DB "2. No. of DTCs = -- ",0Dh,16
MILOFF:DB "MIL status is OFF...",0Dh,00h
MILON:DB "Press to read DTC[~]",0Dh,00h
SETMIL:DB "Clear MIL now?[NO ]~",0Dh,00h
...
TFTPID:DB "221674",0Dh,00h  ;Transmission Fluid Temperature PID 
...
;CGX:DB 11h,0Ah,04h,04h,04h,04h,0Ah,11h,0Dh,00h
CGX:DB 11h,1Bh,0Eh,04h,04h,0Eh,1Bh,11h,0Dh,00h
...
PID22: DB "Trans Fluid:  --- ",0DFh,"c",0Dh,14
