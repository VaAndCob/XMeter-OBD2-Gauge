;Program X-Meter Version 1.0
;Chip    =   AT89S4051
;X-TAL =  22.118 MHz

;Pin Configuration
RS		EQU	P3.4	;0 = Instruction 1 = Data
EN		EQU	P3.5	;High to Low 
BUZZER		EQU	P3.7	;Buzzer Bit

;Internal Ram
PID1		EQU	08h	;PID1 mode
PID2		EQU	09h	;PID2 mode
PIDTURN		EQU	10h	;check which PID will be sent to ELM
WRADDL1	EQU	11h	;LCD write address for calculated value line1
WRADDL2	EQU	12h	;LCD write address for calculated value line2
B1		EQU	13h	;address for Byte1
B2		EQU	14h	;address for Byte2
RESULTLEN	EQU	15h	;keep length of result
CONFIG		EQU	16h	; bit0 = 1 beepon, 0=beep off/
...
MOV	PID1,#05h	;PID mode from memory
MOV	PID2,#05h	;PID mode from memory
METER:
...
JB	INT0,CONSUMP2	;jump back to calculte consump
JB	INT1,CONSUMP2
CALL	SOUND_TRIG
CALL	DEL500m
CALL	DEL500m	;key delay
...
MOV	RESULTLEN,#05
RET
CMDEQV:		;calculate commanded equivalent ratio
MOV	A,B1
MOV	B,#80h
DIV	AB		;Result in A fraction in B
ADD	A,#30h
...
VEHINFO:DB "[Vehicle Serial No.]",0Dh,00h
DTC:DB "[Number of DTCs= --]",0Dh,16
DTCPID:DB "0102",0Dh,00h	;for SBLOCK
...
CONSUM: DB "Fuel Consmp --.-km/L",0Dh,00h
